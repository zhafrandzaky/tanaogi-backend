...
- **Primary Key**: UUID semua tabel (tidak ada auto-increment integer)
...
accommodation_id  UUID FK → accommodations.id (nullable)
...
**Index:**
```sql
UNIQUE INDEX idx_blacklist_ips_ip ON blacklist_ips(ip_address) WHERE is_active = true
...
**Index:**
```sql
UNIQUE INDEX idx_blacklist_phones_phone ON blacklist_phones(phone) WHERE is_active = true
